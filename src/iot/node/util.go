...
//鉴权接口
func (p *Node) auth(id string, plat int, token string) error {
...
func (p *Node) closeConn(conn interface{}) {
	switch conn.(type) {
	case *net.TCPConn:
		c := conn.(*net.TCPConn)
		c.Close()
...
func (p *Node) write(conn interface{}, buf []byte) error {
	var err error
...
func (p *Node) readTimeout(conn *net.TCPConn, len uint32, timeout int) (buffer []byte, err error) {
