...
//Notify 用户状态登记
func (p *RpcClient) Notify(id string, plat int, token string, state int, nodeId string) error {
	logs.Logger.Debug("[rpcclient] report state id=", id, " plat=", plat, " token=", token, " state=", state, " nodeId=", nodeId)
